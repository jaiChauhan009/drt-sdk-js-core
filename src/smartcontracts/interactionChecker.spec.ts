...
describe("integration tests: test checker within interactor", function () {
    ...
    it("should detect errors for 'ultimate answer'", async function () {
        ...
        assert.throw(
            () => {
                let interaction = (<Interaction>contract.methods.getUltimateAnswer()).withValue(
                    TokenTransfer.rewaFromAmount(1),
                );
                checker.checkInteraction(interaction, endpoint);
            },
            errors.ErrContractInteraction,
            "cannot send REWA value to non-payable",
        );
        ...
        assert.throw(
            ...
            Error,
            "Wrong number of arguments for endpoint getUltimateAnswer: expected between 0 and 0 arguments, have 1",
        );
    });
    ...
    it("should detect errors for 'lottery'", async function () {
        ...
        assert.throw(
            () => {
                contract.methods.start(["lucky", TokenTransfer.rewaFromAmount(1)]);
            },
            Error,
            "Wrong number of arguments for endpoint start: expected between 8 and 9 arguments, have 2",
        );
        ...
        assert.throw(
            ...
            errors.ErrContractInteraction,
            "type mismatch at index 4, expected: Option<u64>, got: Option<u32>",
        );
    });
});
